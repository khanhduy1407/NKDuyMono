...
/**
 * The scripts generates no ligature version of NKDuy Mono called NKDuy Mono NL
 *
...
fun main() {
    File("./fonts/ttf/")
            .listFiles { _, name -> name.endsWith(".ttf") && !name.startsWith("NKDuyMonoNL") }
            .forEach {
...
fun generateNoLigaturesFont(file: File, doc: Document) {
    val nlName = file.nameWithoutExtension.replace("NKDuyMono", "NKDuyMonoNL")
    val ttx = "$nlName.ttx"
    val ttf = "$nlName.ttf"
    ...
    nameRecords.forEach {
        if (!it.textContent.contains("trademark")) {
            it.textContent = it.textContent
                .replace("NKDuy Mono", "NKDuy Mono NL")
                .replace("NKDuyMono", "NKDuyMonoNL")
        }
    }
